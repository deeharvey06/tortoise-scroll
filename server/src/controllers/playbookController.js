...
export async function listPlaybooks(req, res) {
  const playbooks = await Playbook.find(ownedFilter(req))
    .sort({ setupName: 1 })
    .lean();
  res.json(playbooks);
}
...
export async function deletePlaybook(req, res) {
  const inUse = await Trade.countDocuments(
    ownedFilter(req, { playbook: req.params.id })
  );
  if (inUse > 0) {
    res.status(409);
    throw new Error(
      `Cannot delete: ${inUse} trade(s) are assigned to this playbook. Reassign or bulk-edit them first.`
    );
  }
  const deleted = await Playbook.findOneAndDelete(
    ownedFilter(req, { _id: req.params.id })
  );
  if (!deleted) {
    res.status(404);
