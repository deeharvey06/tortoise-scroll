...
export async function getTrades(req, res) {
  const result = await tradeService.listTrades(req.query);
  res.json(result);
}
...
export async function deleteTradeHandler(req, res) {
  const deleted = await tradeService.deleteTrade(req.params.id);
  if (!deleted) {
    res.status(404);
    throw new Error(`Trade ${req.params.id} not found`);
  }
...
export async function bulkDeleteHandler(req, res) {
  ...
  const result = await tradeService.bulkDeleteTrades(ids);
...
export async function bulkTagHandler(req, res) {
  ...
  const result = await tradeService.bulkTagTrades(ids, tags);
...
export async function exportTradesCsv(req, res) {
  const trades = await tradeService.exportTrades(req.query);
