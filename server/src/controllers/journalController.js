...
async function validateLinks(req, input) {
  if (
    input.accountId &&
    !(await Account.exists(ownedFilter(req, { _id: input.accountId })))
  ) {
    resNotFound();
  }
  if (
    Array.isArray(input.relatedTrades) &&
    (await Trade.countDocuments(
      ownedFilter(req, { _id: { $in: input.relatedTrades } })
    )) !== new Set(input.relatedTrades.map(String)).size
  )
    resNotFound();
}
