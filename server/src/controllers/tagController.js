...
export async function listTags(req, res) {
  const tags = await Tag.find().sort({ category: 1, name: 1 }).lean();
  res.json(tags);
}
...
export async function createTag(req, res) {
  ...
  const tag = await Tag.findOneAndUpdate(
    { category: category || 'Custom', name: name.trim() },
    { $setOnInsert: { category: category || 'Custom', name: name.trim(), color: color || '' } },
