 ...
 */
export async function generateRiskAlert(accountId) {
  const { settings, current, warnings } = await computeRiskDashboard(accountId || null);
  ...
  const narrative = await narrate(findings, { title: 'Risk Monitor' });
