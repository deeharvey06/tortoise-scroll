...
} from '../scripts/ensureLocalEnv.js';
...
test('recognizes missing, short, and placeholder session secrets as unsafe', () => {
  ...
  assert.equal(
    isUnsafeSessionSecret('replace-with-at-least-32-random-characters'),
    true
  );
  assert.equal(
    isUnsafeSessionSecret('A-real-local-secret-with-more-than-32-characters'),
    false
  );
});
