...
import {
  migrateOwnership,
  OWNED_MODELS,
  repairLegacyRelationships,
} from '../scripts/migrateOwnership.js';
...
test('migration preserves counts, assigns legacy data to ROOT, verifies relationships, and deletes nothing', async () => {
  ...
  stub(User, 'find', () => ({
    lean: async () => [
      {
        _id: rootId,
        emailNormalized: 'root@example.test',
        role: 'ROOT',
        status: 'ACTIVE',
      },
    ],
  ...
  for (const [name, Model] of Object.entries(OWNED_MODELS)) {
    ...
    stub(Model, 'updateMany', async () => {
      const modifiedCount = state[name].unowned;
      state[name].root += modifiedCount;
      state[name].unowned = 0;
    ...
    stub(Model, 'deleteMany', async () => {
      state[name].deleted += 1;
      throw new Error('migration must never delete');
    });
  }
  try {
    ...
    for (const name of Object.keys(OWNED_MODELS)) {
      assert.equal(report.before[name], 2);
      assert.equal(report.after[name], 2);
      assert.equal(report.ownership[name].root, 2);
      assert.equal(report.ownership[name].unowned, 0);
      assert.equal(state[name].deleted, 0);
    }
    assert.ok(
      Object.values(report.relationships).every((result) => result.broken === 0)
    );
  } finally {
...
test('migration preserves a dangling RiskSettings record by converting it to global settings', async () => {
  ...
  const originals = [
    [RiskSettings, 'find', RiskSettings.find],
    [RiskSettings, 'countDocuments', RiskSettings.countDocuments],
    [RiskSettings, 'updateOne', RiskSettings.updateOne],
    [Account, 'find', Account.find],
  ];
  let update;
  RiskSettings.find = () => ({
    select() {
    ...
    lean: async () => [{ _id: settingsId, accountId: missingAccountId }],
  });
  Account.find = () => ({
    select() {
      return this;
  ...
  RiskSettings.updateOne = async (filter, change) => {
