...
const ids = {
  root: new mongoose.Types.ObjectId(),
...
const originals = {
  ...
  deleteSessions: SessionRecord.deleteMany,
};
let calls;
...
after(() => {
  ...
  SessionRecord.deleteMany = originals.deleteSessions;
...
function stubTarget(targetRole = 'USER', targetStatus = 'ACTIVE') {
  ...
  User.findOne = (filter) => {
    calls.find += 1;
    const value =
      filter.role?.$ne === 'ROOT' && targetRole === 'ROOT'
        ? null
        : { ...target };
    return {
      lean: async () => value,
...
test('USER has no administration access', async () => {
  stubTarget();
  const response = await request(globalThis.adminTestApp)
    .get(`/api/admin/users/${ids.user}`)
...
test('ADMIN cannot change roles or statuses', async () => {
  ...
  const role = await request(globalThis.adminTestApp)
    ...
    .send({ role: 'ADMIN' });
  const status = await request(globalThis.adminTestApp)
...
test('ADMIN cannot view, create, delete, or otherwise target ROOT', async () => {
  stubTarget('ROOT');
  const view = await request(globalThis.adminTestApp)
    .get(`/api/admin/users/${ids.user}`)
    .set('x-test-role', 'ADMIN');
  const create = await request(globalThis.adminTestApp)
    .post('/api/admin/users')
    .set('x-test-role', 'ADMIN')
    .send({ role: 'ROOT' });
  const remove = await request(globalThis.adminTestApp)
    .delete(`/api/admin/users/${ids.user}`)
    .set('x-test-role', 'ADMIN');
