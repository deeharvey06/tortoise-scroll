...
function DataPoint({ label, children }) {
  return (
    <Box>
      ...
      <Box className='mono-data' sx={{ minHeight: 24, fontWeight: 600 }}>
        {children ?? '—'}
      </Box>
    </Box>
  );
...
export default function TradeDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setJournal({
        ...
        missedOpportunity: !!value.missedOpportunity,
        tags: value.tags || [],
      });
    } catch (err) {
      setError(
  ...
  useEffect(() => {
    ...
    tagApi
      .fetchTags()
      .then((tags) => setAllTags(tags.map((tag) => tag.name)))
      .catch(() => {});
  }, [load]);
  ...
  return (
    <Box sx={{ maxWidth: 1280, mx: 'auto' }}>
      <PageHeader
        ...
        title={trade.symbol}
        description={`${format(new Date(trade.entryTime), 'PPP · p')}${trade.exitTime ? ` → ${format(new Date(trade.exitTime), 'PPP · p')}` : ' · Open position'}`}
        actions={
      ...
      {toast && (
        <Alert severity='success' onClose={() => setToast(null)} sx={{ mb: 4 }}>
          {toast}
        </Alert>
      )}
      ...
      <SectionHeader
        eyebrow='Summary'
        title='Trade outcome'
        description='The result, risk, and time committed to this trade.'
      />
      <Grid container spacing={2} sx={{ mb: 5 }}>
        <Grid item xs={12} sm={6} md={3}>
          <MetricCard
            label='Net P&L'
            value={money(trade.netPnL)}
            tone={
              trade.netPnL > 0
                ? 'positive'
                : trade.netPnL < 0
        ...
        <Grid item xs={12} sm={6} md={3}>
          <MetricCard label='Risk' value={money(trade.riskAmount)} />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <MetricCard
            label='Duration'
            value={duration(trade.holdingTimeSeconds)}
          />
        </Grid>
      </Grid>
        ...
        <Grid item xs={12} lg={8}>
          ...
          <Panel>
            ...
            <Grid container spacing={3}>
              <Grid item xs={6} sm={3}>
                <DataPoint label='Quantity'>{trade.quantity}</DataPoint>
              </Grid>
              <Grid item xs={6} sm={3}>
              ...
              <Grid item xs={6} sm={3}>
                <DataPoint label='Stop loss'>{trade.stopLoss ?? '—'}</DataPoint>
              </Grid>
              <Grid item xs={6} sm={3}>
                <DataPoint label='Gross P&L'>
                  <ProfitLossValue value={trade.grossPnL} />
                </DataPoint>
              </Grid>
              <Grid item xs={6} sm={3}>
                <DataPoint label='Fees'>{money(trade.fees || 0)}</DataPoint>
              </Grid>
              <Grid item xs={6} sm={3}>
                <DataPoint label='Commission'>
                  {money(trade.commission || 0)}
                </DataPoint>
              </Grid>
              <Grid item xs={6} sm={3}>
                <DataPoint label='Take profit'>
                  {trade.takeProfit ?? '—'}
                </DataPoint>
              </Grid>
            </Grid>
            {trade.executions?.length ? (
              ...
              >
                <Table size='small' aria-label='Trade executions'>
                  <TableHead>
                    <TableRow>
                  ...
                  <TableBody>
                    {trade.executions.map((execution, index) => (
                      <TableRow key={`${execution.time}-${index}`}>
                        <TableCell
                          className='mono-data'
                          sx={{ whiteSpace: 'nowrap' }}
                        >
                          {format(
                            new Date(execution.time),
                            'MMM d, yyyy HH:mm:ss'
                          )}
                        </TableCell>
                        <TableCell>
              ...
              <Typography variant='body2' color='text.secondary' sx={{ mt: 3 }}>
                Single entry / single exit — no individual fills recorded.
              </Typography>
            )}
          </Panel>
        </Grid>
        <Grid item xs={12} lg={4}>
          <Panel sx={{ height: '100%' }}>
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={6}>
                <DataPoint label='Session'>{trade.session || '—'}</DataPoint>
              </Grid>
              <Grid item xs={6}>
                <DataPoint label='Asset type'>
                  {trade.assetType || '—'}
                </DataPoint>
              </Grid>
              <Grid item xs={6}>
                <DataPoint label='Timeframe'>
                  {trade.timeframe || '—'}
                </DataPoint>
              </Grid>
              <Grid item xs={12}>
                <DataPoint label='Market'>{trade.market || '—'}</DataPoint>
              </Grid>
              <Grid item xs={12}>
                ...
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mt: 1 }}>
                  {trade.tags?.length
                    ? trade.tags.map((tag) => <Tag key={tag} label={tag} />)
                    : '—'}
                </Box>
              </Grid>
            </Grid>
        ...
        <Grid item xs={12} lg={7}>
          <Panel sx={{ height: '100%' }}>
            <SectionHeader
              ...
              description='Capture the reasoning and market conditions surrounding the trade.'
            />
            <Grid container spacing={2}>
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  onChange={(e) => updateJournal('entryReason', e.target.value)}
                />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  onChange={(e) => updateJournal('exitReason', e.target.value)}
                />
              </Grid>
              <Grid item xs={12}>
                <TextField
                  ...
                  onChange={(e) =>
                    updateJournal('marketCondition', e.target.value)
                  }
                />
              </Grid>
              <Grid item xs={12}>
              ...
              <Grid item xs={12}>
                <Autocomplete
                  ...
                  options={allTags}
                  value={journal.tags}
                  onChange={(e, value) => updateJournal('tags', value)}
        ...
        <Grid item xs={12} lg={5}>
          <Panel sx={{ height: '100%' }}>
            <SectionHeader
              ...
              title='Discipline and self-assessment'
            />
            <Grid container spacing={2}>
              <Grid item xs={12}>
                <Autocomplete
                  ...
                  onChange={(e, value) => updateJournal('mistake', value)}
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      label='Mistakes'
              ...
              <Grid item xs={12}>
                <Autocomplete
                  ...
                  onChange={(e, value) => updateJournal('emotion', value)}
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      label='Emotions'
              ...
              <Grid item xs={12}>
                ...
                <Rating
                  value={journal.confidence || 0}
                  onChange={(e, value) => updateJournal('confidence', value)}
                />
              </Grid>
              <Grid item xs={12}>
                <Box sx={{ display: 'grid' }}>
                  <FormControlLabel
                    control={
                      <Checkbox
                        ...
                        onChange={(e) =>
                          updateJournal('followedPlan', e.target.checked)
                        }
                      />
                    }
                    label='Followed plan'
                  ...
                  <FormControlLabel
                    control={
                      <Checkbox
                        ...
                        onChange={(e) =>
                          updateJournal('followedRules', e.target.checked)
                        }
                      />
                    }
                  ...
                  <FormControlLabel
                    control={
                      <Checkbox
                        ...
                        onChange={(e) =>
                          updateJournal('missedOpportunity', e.target.checked)
                        }
                      />
                    }
      ...
      <Panel>
        <SectionHeader
          eyebrow='Screenshots'
          title='Visual record'
          description='Charts and execution evidence attached to this trade.'
          actions={
        ...
        {!trade.screenshots?.length ? (
          <EmptyState
            ...
            description='Upload a chart or execution screenshot to preserve the visual context.'
          />
        ) : (
          <Grid container spacing={2}>
                ...
                >
                  ...
                  <Box sx={{ p: 1.5, display: 'flex', gap: 1 }}>
                    <TextField
                      ...
                      onBlur={(e) => caption(shot._id, e.target.value)}
                    />
                    <IconButton
                      size='small'
                      aria-label='Delete screenshot'
                      onClick={() => removeShot(shot._id)}
                    >
