  ...
  return (
    <Panel sx={{ maxWidth: 720 }}>
      <SectionHeader
        ...
        description='Set the display context used throughout your journal.'
      />
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} sm={6}>
          <TextField
            ...
            onChange={(e) =>
              setForm((f) => ({
                ...f,
                defaultAccountId: e.target.value || null,
              }))
            }
          >
        ...
        <Grid item xs={12} sm={6}>
          <TextField
            label='Trading hours start'
            fullWidth
            size='small'
            value={form.tradingHoursStart}
            onChange={(e) =>
              setForm((f) => ({ ...f, tradingHoursStart: e.target.value }))
            }
          />
        </Grid>
        <Grid item xs={12} sm={6}>
          <TextField
            label='Trading hours end'
            fullWidth
            size='small'
            value={form.tradingHoursEnd}
            onChange={(e) =>
              setForm((f) => ({ ...f, tradingHoursEnd: e.target.value }))
            }
          />
        </Grid>
      </Grid>
  ...
  return (
    <Panel sx={{ maxWidth: 720 }}>
      <SectionHeader
        ...
        description='Defaults accelerate entry without changing trade calculations.'
      />
      <Grid container spacing={2}>
        <Grid item xs={12} sm={6}>
          <TextField
        ...
        <Grid item xs={12}>
          <TextField
            ...
            fullWidth
            size='small'
            value={form.defaultStrategyId || ''}
            onChange={(e) =>
              setForm((f) => ({
                ...f,
                defaultStrategyId: e.target.value || null,
              }))
            }
          >
            <MenuItem value=''>None</MenuItem>
  ...
  return (
    <Panel sx={{ maxWidth: 760 }}>
      <SectionHeader
        ...
        description='Maintain the consistent labels used to classify process and performance.'
      />
      <Typography variant='body2' color='text.secondary' sx={{ mb: 2 }}>
        Tags applied on trades are free-form, but this catalog powers
        autocomplete and lets you manage the vocabulary by category (Setup /
        Mistake / Emotion / Custom).
      </Typography>
      {error && (
  ...
  return (
    <Box sx={{ maxWidth: 700 }}>
      <Panel sx={{ mb: 4 }}>
        <SectionHeader
          title='Export'
          description='Create a portable copy of your journal data.'
        />
        <Typography variant='body2' color='text.secondary' sx={{ mb: 2 }}>
          Download your entire database (accounts, trades, strategies,
          playbooks, tags, journal, risk settings, backtests, AI
          memories/conversations) as a single JSON file. Screenshot image files
          are not included — back up <code>server/uploads</code> separately to
          preserve those. Your OpenAI API key is never included.
        </Typography>
      ...
      <Panel sx={{ mb: 4 }}>
        <SectionHeader
          title='Restore'
          description='Replace current data from a previously exported Tortoise Scroll backup.'
        />
        <Alert severity='warning' sx={{ mb: 2 }}>
          Restoring replaces every collection present in the backup file with
          its contents. This cannot be undone — export a fresh backup first if
          ...
          <Box sx={{ mt: 2 }}>
            ...
            {result.report.map((r) => (
              ...
              >
                ...
                {r.error
                  ? `failed — ${r.error}`
                  : `${r.restored} document(s) restored`}
              </Typography>
            ))}
          </Box>
        )}
      ...
      <Panel>
        ...
        <Typography variant='body2' color='text.secondary' sx={{ mb: 1 }}>
          For a complete binary backup (including indexes), use MongoDB's own
...
mongorestore --uri="mongodb://localhost:27017/trading-journal" ./backup/trading-journal`}
      ...
      <ConfirmationDialog
        ...
        confirmLabel='Restore and replace my data'
  ...
  return (
    <Box>
      <PageHeader
        eyebrow='System'
        title='Settings'
        description='Manage Tortoise Scroll preferences, trading defaults, vocabulary, intelligence, and data.'
      ...
      {tab === 'ai' && (
        <Panel sx={{ maxWidth: 620 }}>
          <SectionHeader
            title='Tortoise AI settings'
            description='Provider controls remain beside the AI experience so changes can be evaluated immediately.'
          />
          <Typography variant='body2' sx={{ mb: 2 }}>
            AI provider, model, and temperature settings live on the AI Trading
            Partner page itself, since that's where you can immediately see the
            effect of a change.
          </Typography>
          <Button
