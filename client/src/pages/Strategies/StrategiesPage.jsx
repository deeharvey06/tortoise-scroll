...
export default function StrategiesPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([metrics, trades]) => {
        ...
        setAssociatedTrades(trades.items);
      })
      .catch((err) =>
        setError(err.response?.data?.error?.message || err.message)
      )
  ...
  return (
    <Box>
      <PageHeader
        eyebrow='Edge library'
        title='Strategies'
        description='Define how you trade, codify the rules, and validate each strategy against its recorded evidence.'
        actions={
          <Button
            variant='contained'
            size='small'
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description='Create a strategy, document its rules, then assign trades to it from the Trade form.'
          action={
                ...
                {strategies.map((s) => (
                  ...
                  >
                    <ListItemText
                      primary={s.name}
                      secondary={
                        [s.market, s.timeframe].filter(Boolean).join(' · ') ||
                        undefined
                      }
                      primaryTypographyProps={{
                        fontWeight: s._id === selectedId ? 700 : 500,
                      }}
                    />
                  </ListItemButton>
                ))}
            ...
            {selected && (
              <>
                ...
                >
                  <Box>
                    ...
                    <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
                      {selected.market && (
                        <StatusBadge label={selected.market} />
                      )}
                      {selected.timeframe && (
                        <StatusBadge label={selected.timeframe} tone='info' />
                      )}
                    </Box>
                  </Box>
                  <Box>
                ...
                <SectionHeader
                  ...
                  description='Evidence calculated from trades assigned to this strategy.'
                />
                {perfLoading ? (
                  ...
                  <Grid container spacing={1.5} sx={{ mb: 5 }}>
                    <Grid item xs={6} sm={4} lg={2}>
                      <KpiCard
                        label='Net P&L'
                        value={fmtMoney(performance.netPnL)}
                        colorByValue
                      />
                    </Grid>
                    <Grid item xs={6} sm={4} lg={2}>
                    ...
                    <Grid item xs={6} sm={4} lg={2}>
                      <KpiCard
                        label='Expectancy'
                        value={fmtMoney(performance.expectancy)}
                        colorByValue
                      />
                    </Grid>
                    <Grid item xs={6} sm={4} lg={2}>
                    ...
                    <Grid item xs={6} sm={4} lg={2}>
                      <KpiCard
                        label='Avg winner'
                        value={fmtMoney(performance.avgWin)}
                        colorByValue
                      />
                    </Grid>
                    <Grid item xs={6} sm={4} lg={2}>
                      <KpiCard
                        label='Avg loser'
                        value={fmtMoney(performance.avgLoss)}
                        colorByValue
                      />
                    </Grid>
                    <Grid item xs={6} sm={4} lg={2}>
                      <KpiCard
                        label='Max drawdown'
                        value={fmtMoney(performance.maxDrawdown)}
                        colorByValue
                      />
                    </Grid>
                  </Grid>
                  ...
                  <Grid item xs={12} lg={5}>
                    <Panel sx={{ height: '100%' }}>
                      ...
                      {selected.description ? (
                        ...
                        <EmptyState
                          compact
                          title='Definition not documented'
                          description='Edit this strategy to record its purpose and market context.'
                        />
                      )}
                ...
                <Panel sx={{ mb: 3 }}>
                  ...
                  >
                    <SectionHeader
                      ...
                      description='Visual examples supporting the written strategy.'
                      sx={{ mb: 0 }}
                    />
                    <Button
                      size='small'
                  ...
                  {(selected.screenshots || []).length === 0 ? (
                    <EmptyState
                      compact
                      title='No reference screenshots'
                      description='Upload only examples that clarify or validate the strategy.'
                    />
                  ) : (
                        ...
                        <Grid item xs={12} sm={6} md={4} key={shot._id}>
                          ...
                          >
                            <Box
                              component='img'
                              src={shot.url}
                              alt={shot.caption || `${selected.name} reference`}
                              sx={{
                                width: '100%',
                                height: 160,
                                objectFit: 'cover',
                            ...
                            >
                              <IconButton
                                ...
                                onClick={() => handleImageDelete(shot._id)}
                              >
                                <DeleteIcon fontSize='small' />
                              </IconButton>
                            </Box>
                ...
                <Panel padding={0}>
                  <Box sx={{ p: 3 }}>
                    <SectionHeader
                      eyebrow='Evidence'
                      title='Associated trades'
                      description={`The ${Math.min(associatedTrades.length, 8)} most recent trades assigned to this strategy.`}
                      sx={{ mb: 0 }}
                    />
                  </Box>
                  {associatedTrades.length ? (
                    <TableContainer>
                      <Table size='small'>
                        <TableHead>
                          <TableRow>
                    ...
                    <EmptyState
                      compact
                      title='No associated trades'
                      description='Assign trades to this strategy from the Trade form to build an evidence base.'
                    />
                  )}
                </Panel>
              </>
          ...
          <Grid container spacing={2}>
            {FIELDS.map(([key, label, multiline]) => (
              <Grid
                item
                xs={12}
                sm={key === 'market' || key === 'timeframe' ? 6 : 12}
                key={key}
              >
                <TextField
                  label={label}
