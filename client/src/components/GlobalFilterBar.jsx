  ...
  return (
    ...
    >
      ...
      {filters.datePreset === 'custom' && !compact && (
        <>
          <TextField
            ...
            onChange={(e) =>
              filters.setCustomRange(e.target.value, filters.customTo)
            }
            sx={{ width: 150 }}
          />
          <TextField
            ...
            onChange={(e) =>
              filters.setCustomRange(filters.customFrom, e.target.value)
            }
            sx={{ width: 150 }}
          />
        </>
      ...
      >
        <Stack
          spacing={3}
          sx={{ p: 4, width: compact ? 'min(320px, calc(100vw - 32px))' : 280 }}
        >
          ...
          <TextField
            ...
            label='Setup'
            value={filters.setup}
            onChange={(e) => filters.setSetup(e.target.value)}
          />
      ...
      {activeCount > 0 && !compact && (
        <Stack
          direction='row'
          spacing={1}
          sx={{ minWidth: 0, overflowX: 'auto', scrollbarWidth: 'none' }}
        >
          {filters.accountId && (
            <Tag
              label={
                accounts.find((a) => a._id === filters.accountId)?.name ||
                'account'
              }
              onDelete={() => filters.setAccountId('')}
            />
          )}
          {filters.symbol && (
            <Tag
              label={filters.symbol}
              onDelete={() => filters.setSymbol('')}
            />
          )}
          {filters.setup && (
            <Tag label={filters.setup} onDelete={() => filters.setSetup('')} />
          )}
          {filters.direction && (
            <Tag
              label={filters.direction}
              onDelete={() => filters.setDirection('')}
            />
          )}
          {filters.session && (
            <Tag
              label={filters.session}
              onDelete={() => filters.setSession('')}
            />
          )}
        </Stack>
      )}
