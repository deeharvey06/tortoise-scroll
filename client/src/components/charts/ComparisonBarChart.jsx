  ...
  return (
    <Box sx={{ ...tooltipStyle, p: 2.5 }}>
      ...
      <Typography variant='caption' display='block'>
        Net P&amp;L: {row.netPnL < 0 ? '−' : '+'}$
        {Math.abs(row.netPnL).toFixed(2)}
      </Typography>
      <Typography variant='caption' display='block'>
        Sample: {row.count} trade{row.count === 1 ? '' : 's'}
      </Typography>
      {row.winRate != null && (
        <Typography variant='caption' display='block'>
      ...
      {row.avgR != null && (
        <Typography variant='caption' display='block'>
          Average R: {row.avgR.toFixed(2)}R
        </Typography>
      )}
    </Box>
  );
...
export default function ComparisonBarChart({ rows = [], height = 260 }) {
  ...
  return (
    <Box
      sx={{ width: '100%', height }}
      role='img'
      aria-label={`Net profit and loss comparison across ${data.length} groups`}
    >
      <ResponsiveContainer width='100%' height='100%'>
        ...
        >
          <CartesianGrid
            strokeDasharray='3 3'
            stroke='var(--ts-chart-grid)'
            horizontal={false}
          />
          <XAxis
            type='number'
            tick={{ fill: 'var(--ts-chart-axis)', fontSize: 11 }}
          ...
          <Bar dataKey='netPnL' radius={[0, 3, 3, 0]} isAnimationActive={false}>
            {data.map((row) => (
              <Cell
                ...
                fill={
                  row.netPnL >= 0
                    ? 'var(--ts-financial-positive)'
                    : 'var(--ts-financial-negative)'
                }
              />
            ))}
          </Bar>
