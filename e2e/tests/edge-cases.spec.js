...
test("Thinkorswim execution import reconstructs positions and is idempotent", async ({
  page,
}) => {
  const account = await createAccount(page, `E2E TOS Import ${Date.now()}`);
  const tosCsv = path.join(
    __dirname,
    "..",
    "fixtures",
    "thinkorswim-executions.csv",
  );

  await page.goto("/import");
  await selectMuiOptionByLabel(page, "Broker format", "Thinkorswim");
  await uploadViaLabelButton(page, "Choose file", tosCsv);
  await page.getByRole("button", { name: "Preview" }).click();

  await expect(page.getByText("Execution format detected")).toBeVisible();
  await expect(page.getByText(/3 valid executions/)).toBeVisible();
  await page.getByRole("button", { name: "Continue" }).click();
  await selectMuiOptionByLabel(page, "Import into account", account.name);
  await page.getByRole("button", { name: /^Import 3 rows$/ }).click();

  await expect(page.getByText("1 new trades")).toBeVisible();
  await expect(page.getByText("3 executions detected")).toBeVisible();
  await expect(page.getByText("3 executions added")).toBeVisible();
  await expect(page.getByText("1 trades reconstructed")).toBeVisible();
  await expect(page.getByText("0 open positions")).toBeVisible();
  ...
  await expect(page.getByText("0 new trades")).toBeVisible();
  await expect(page.getByText("3 duplicate executions skipped")).toBeVisible();
  await expect(page.getByText("0 executions added")).toBeVisible();
});
